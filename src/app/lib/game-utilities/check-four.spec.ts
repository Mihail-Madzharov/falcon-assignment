...
describe("Check for a winner move", () => {
  describe("- initial game state", () => {
    it("has no winning move", () => {
      ...
      const noWinner = board.every((col, colIndex) =>
        col.every(row => {
          return !(
            checkForPlayerOne(board, [colIndex, row]) ||
            checkForPlayerTwo(board, [colIndex, row])
          );
        })
      );
      expect(noWinner).toBeTruthy();
    });
  });

  describe("- non-winning move", () => {
    it("returns null if last move is the very first move", () => {
      const board = generateMatrixModel(6, 7);
      board[0][0] = 1; // <- simulate move by player one
      const lastMove = [0, 0];
      expect(checkForPlayerOne(board, lastMove)).toBeNull();
    });
  ...
  describe("- winning move", () => {
    it("works with columns", () => {
      ...
      board[0][3] = 1;
      const lastMove = [3, 0];
      expect(checkForPlayerOne(board, lastMove)).toEqual(
        jasmine.any(WinningMove)
      );
    });
